        ...
        <div v-for="(message, index) in messages" :key="index">
          ...
          >
            ...
            <div class="flex items-start">
              <div
                class="bg-gray-100 rounded-2xl p-4 text-sm whitespace-pre-line shadow-sm break-words"
                style="
                  max-width: 80%;
                  border-radius: 18px;
    ...
    const processBookingInfo = async (userInput) => {
      try {
        ...
        if (extractedInfo.paymentMethod) {
          ...
          const matchedPayment = paymentMethods.value.find(
            (p) =>
              p.nama.toLowerCase().includes(paymentName) ||
              paymentName.includes(p.nama.toLowerCase())
          );

          if (matchedPayment) {
            collectedInfo.paymentMethodId = matchedPayment.id;
            collectedInfo.paymentMethodName = matchedPayment.nama;
            console.log("✅ Payment method matched:", matchedPayment.nama);
          } else {
            console.log("❌ No payment method match for:", paymentName);
          }
        }
...
User message: "${userInput}"`;
    ...
    const submitBooking = async () => {
      try {
        ...
        if (isCashlessSelected && transactionResult.snap_url) {
          const paymentUrl = transactionResult.snap_url;
          addBotMessage(
    ...
    const handleDirectDateInput = (userText) => {
      ...
      // Handle common date patterns directly - improved regex
      const datePattern =
        /^(\d{1,2})\s+(januari|februari|maret|april|mei|juni|juli|agustus|september|oktober|november|desember|jan|feb|mar|apr|mei|jun|jul|agt|aug|sep|okt|oct|nov|des|dec)$/i;

      if (userText.match(datePattern)) {
        // Extract the day number and month
        const match = userText.match(datePattern);
        if (match) {
          const day = parseInt(match[1], 10);
          const monthStr = match[2].toLowerCase();
          ...
          if (!isNaN(day) && day >= 1 && day <= 31) {
            ...
            if (months.hasOwnProperty(monthStr)) {
              ...
              console.log(
                `Direct date input: day=${day}, month=${month} (${monthStr}), year=${year}`
              );
    ...
    // Direct payment method handler
    const handleDirectPaymentInput = (userText) => {
      console.log("💰 Processing payment method input:", userText);

      // Normalize input
      const normalizedInput = userText.toLowerCase().trim();

      // Check for cash payment
      if (normalizedInput === "cash" || normalizedInput.includes("tunai")) {
        return paymentMethods.value.find(
          (method) =>
            method.nama.toLowerCase() === "cash" ||
            method.nama.toLowerCase() === "tunai"
        );
      }

      // Check for cashless payment
      if (
        normalizedInput === "cashless" ||
        normalizedInput.includes("transfer") ||
        normalizedInput.includes("online") ||
        normalizedInput.includes("e-wallet")
      ) {
        return paymentMethods.value.find(
          (method) =>
            method.nama.toLowerCase() === "cashless" ||
            method.nama.toLowerCase().includes("online")
        );
      }

      // Try to match with available payment methods
      return paymentMethods.value.find(
        (method) =>
          normalizedInput.includes(method.nama.toLowerCase()) ||
          method.nama.toLowerCase().includes(normalizedInput)
      );
    };
    ...
    const sendMessage = async () => {
      ...
      // Special case for date input - handle this BEFORE time processing
      if (
        bookingState.value.isBookingFlow &&
        (bookingState.value.currentStep === "date" ||
          !bookingState.value.collectedInfo.date)
        ...
        if (
          ...
          userText.toLowerCase().includes("januari") ||
          userText.toLowerCase().includes("februari") ||
          userText.toLowerCase().includes("maret") ||
          userText.toLowerCase().includes("april") ||
          userText.toLowerCase().includes("mei") ||
          userText.toLowerCase().includes("juli") ||
          userText.toLowerCase().includes("agustus") ||
          userText.toLowerCase().includes("september") ||
          userText.toLowerCase().includes("oktober") ||
          userText.toLowerCase().includes("november") ||
          userText.toLowerCase().includes("desember") ||
          userText.toLowerCase().includes("aku bilang") ||
          userText.toLowerCase().includes("sudah bilang")
        ) {
          console.log("🔄 User seems to be providing date information");
          ...
          if (numbers && numbers.length > 0) {
            ...
            if (month >= 0 && !isNaN(day)) {
              ...
              if (!isNaN(date.getTime()) && date.getDate() === day) {
                ...
                bookingState.value.collectedInfo.date = dateStr;
                console.log("✅ Forced date recognition successful:", dateStr);
      ...
      // Special case for time input - handle this AFTER date processing
      if (
        bookingState.value.isBookingFlow &&
        bookingState.value.currentStep === "time" &&
        bookingState.value.collectedInfo.date && // Only process time if we already have a date
      ...
      // Special case for payment method input
      if (
        bookingState.value.isBookingFlow &&
        (bookingState.value.currentStep === "payment" ||
          !bookingState.value.collectedInfo.paymentMethodId)
      ) {
        console.log("💰 Processing payment method input:", userText);

        // Try to recognize payment method directly
        const recognizedPayment = handleDirectPaymentInput(userText);
        ...
        if (recognizedPayment) {
          ...
          // Set the payment method directly
          bookingState.value.collectedInfo.paymentMethodId =
            recognizedPayment.id;
          bookingState.value.collectedInfo.paymentMethodName =
            recognizedPayment.nama;
          console.log(
      ...
      try {
        ...
        if (bookingState.value.isBookingFlow) {
          ...
          if (bookingState.value.currentStep === "confirm") {
            // Direct check for common confirmation words in Indonesian
            const confirmWords = [
              "ya",
              "iya",
              "yes",
            ...
            // Check if input directly matches confirmation words
            if (
              confirmWords.some(
                (word) =>
                  normalizedInput === word || normalizedInput.includes(word)
              )
            ) {
              console.log("✅ User confirmed booking with:", normalizedInput);
              await submitBooking();
              return;
            }

            // Check if input directly matches cancellation words
            if (
              cancelWords.some(
                (word) =>
                  normalizedInput === word || normalizedInput.includes(word)
              ...
              console.log("❌ User canceled booking with:", normalizedInput);
              resetBookingState();
              addBotMessage(
            ...
            // If no direct match, fall back to AI processing
            console.log(
              "🤔 Using AI to determine if user is confirming or canceling"
            );
            ...
            console.log("AI confirmation response:", confirmResponse);
